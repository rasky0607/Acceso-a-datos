...
//IMPORTANTE
    ...
    public void SelectClienteInversio(Connection c) throws SQLException {

        String Query = "SELECT id_cliente,dni,nom_cliente from cliente";
        PreparedStatement psSelect = c.prepareStatement(Query);
        ResultSet rs = psSelect.executeQuery();
        rs.last();//Colocamos el cursor en la ultima posicion

        //Leemos con un do while por que primero leemos en la posion en la que esta el cursor
        //y luego retrocedemos una posicion con previus()
       do {
            System.out.println("ID: " + rs.getInt("id_cliente"));
            System.out.println("DNI: " + rs.getString("dni"));
            System.out.println("Nombre: " + rs.getString("nom_cliente"));
